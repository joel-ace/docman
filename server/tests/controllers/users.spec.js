...
describe('Users', () => {
  describe('creating a new account ', () => {
    it(
      'should get a status code of 201, token and user array if successful',
      (done) => {
        chai.request(app)
          .post('/api/v1/users')
          .send(
    ...
    it(
      'should get an error message and status code 409 if user already exists',
      (done) => {
        chai.request(app)
          .post('/api/v1/users')
          .send({
  ...
  describe('during login', () => {
    it(
      `should get an error message and status code 401
      if email supplied does not exist`,
      (done) => {
        chai.request(app)
    ...
    it(
      'should get a status code of 200 and a token if login is successful',
      (done) => {
        chai.request(app)
          .post('/api/v1/users/login')
          .send({
  ...
  describe('viewing all users', () => {
    ...
    it(
      'should receive a status code of 200 and an array of user object',
      (done) => {
        chai.request(app)
          .get('/api/v1/users')
          .set({ Authorization: adminToken })
    ...
    it(
      'should get and paginate all users when limit and offset are supplied',
      (done) => {
        chai.request(app)
  ...
  describe('viewing a user using the user id', () => {
    it(
      `should get a status code of 404 and error message
  ...
  describe('searching for users using name', () => {
    it(
      `should get a status code of 200 and an array of user object
      if search query matches a user`,
      (done) => {
        chai.request(app)
  ...
  describe('updating a user using the user id', () => {
    ...
    it(
      `should get a status code of 403 
      if user requesting to update details is not account owner`,
      (done) => {
        chai.request(app)
          .put('/api/v1/users/1')
    ...
    it(
      'should receive a status of 200 if user details is successfully updated',
      (done) => {
        chai.request(app)
          .put('/api/v1/users/3')
          .send({
    ...
    it(
      `should receive a status of 400 
      if old password is not provided for password change`,
      (done) => {
        chai.request(app)
          .put('/api/v1/users/3')
          .send({
    ...
    it(
      `should receive a status of 403 
      if old password provided is not user password`,
      (done) => {
        chai.request(app)
          .put('/api/v1/users/3')
          .send({
  ...
  describe('viewing a document', () => {
    it(
      `should get a status code of 404 and an error message 
      if user id does not belong to a valid user`,
      (done) => {
        chai.request(app)
    ...
    it(
      'should get documents belonging to the user with the supplied userId',
      (done) => {
        chai.request(app)
  ...
  describe('deleting a user', () => {
    it('should get a status code of 403 if user is not admin', (done) => {
      chai.request(app)
        ...
        .end((err, res) => {
          expect(res.status).to.equal(403);
          expect(res.body.message).to.equal(
            'Only the owner or an admin can access this resource'
          );
          done();
        });
    });
    it(
      `should get a status code of 403 
      if an attempt is made to delete the admin account by the admin`,
      (done) => {
        chai.request(app)
          .delete('/api/v1/users/1')
          .set({ Authorization: adminToken })
    ...
    it(
      'should receive a status of 200 if user is successfully deleted',
      (done) => {
        chai.request(app)
    ...
    it(
      `should receive a status of 401 
      if user making the request does not exist in database`,
      (done) => {
        chai.request(app)
          .delete('/api/v1/users/3')
          .set({ Authorization: userToken })
    ...
    it(
      'should receive a status of 404 if user id does not exist in database',
      (done) => {
        chai.request(app)
          .delete('/api/v1/users/3')
          .set({ Authorization: adminToken })
